...
class ImgRecRPC:
    ...
    def __init__(self, channel: BlockingChannel) -> None:
        channel.exchange_declare(exchange="img_rec_rpc_send", exchange_type="direct")

        channel.exchange_declare(exchange="img_rec_rpc_rec", exchange_type="direct")
        result = channel.queue_declare("", exclusive=True)
        self.callback_queue = result.method.queue
        channel.queue_bind(
            exchange="img_rec_rpc_rec", queue=self.callback_queue, routing_key="response"
        )
